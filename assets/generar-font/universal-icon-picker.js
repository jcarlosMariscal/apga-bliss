var iconPickerUrl = document.currentScript.src.replace(
  /js\/([a-z\.-]+)$/gm,
  ""
);
...
(function (root, factory) {
  ...
  function UniversalIconPicker(selector, options) {
    ...
    this.universalWrap =
      ...
      "assets/generar-font" +
      '/images/xmark-solid.svg" width="20" height="16" alt="Close" title="Close" /></div></div><div class="uip-modal--body"><div id="uip-modal--sidebar' +
      this.idSuffix +
      ...
      "assets/generar-font" +
      '/images/magnifying-glass-solid.svg" width="20" height="16" alt="Search" title="Search" /></div><div class="uip-modal--icon-preview-inner"><div id="uip-modal--icon-preview' +
      this.idSuffix +
    ...
    if (this.options.mode === "autoload") {
      ...
      document
        ...
        .addEventListener("click", this.init.bind(this), { once: true });
    }
  }
  ...
  UniversalIconPicker.prototype = {
      ...
      this._loadIconLibraries().then(() => {
        ...
        if (!document.getElementById("uip-modal" + this.idSuffix)) {
          ...
          this.universalDomEle
            ...
            .addEventListener("click", () => {
              let selected = this.universalDomEle.querySelector(
                ".universal-selected"
              );
              ...
              if (selected) {
                ...
                if (
                  ...
                  jsonOutput.iconClass =
                    selected.querySelector("i").classList.value;
    ...
    _loadCssFiles: function () {
      ...
      if (!loadedDependencies.includes("universal-icon-picker.min.css")) {
        ...
        link.type = "text/css";
        link.href =
          "assets/generar-font/stylesheets/universal-icon-picker.min.css";
        link.media = "screen";
      ...
      if (this.options.iconLibrariesCss) {
        this.options.iconLibrariesCss.forEach((cssFile) => {
          if (!loadedDependencies.includes(cssFile)) {
            let cssFileLink = "assets/generar-font/stylesheets/" + cssFile;
    ...
    _loadIconLibraries: async function (i = 0) {
      ...
      await fetch("assets/generar-font/icons-libraries/" + iconLib)
        .then((response) => response.json())
        .then((data) => {
          // Success!
          var camelCasedIconLibrary = iconLib
      ...
      if (libraryContent.icons !== undefined) {
        ...
        Object.entries(libraryContent).forEach((item) => {
          listItem = {
            ...
            "list-icon":
              item[1]["list-icon"] !== undefined ? item[1]["list-icon"] : "",
            "library-id":
      ...
      sideBarList.forEach((item) => {
        ...
        if ("all" !== item["library-id"]) {
          ...
          if (item["prefix"].match(/^material-icons/)) {
            iconTag =
              '<i class="' + item["prefix"] + '">' + item["list-icon"] + "</i>";
          }
          markup +=
          ...
          markup +=
            ...
            '"><img src="' +
            "assets/generar-font" +
            '/images/star-of-life-solid.svg" width="13.125px" height="auto" alt="All" title="All" />' +
            item["title"] +
            "</div>";
        }
      });
